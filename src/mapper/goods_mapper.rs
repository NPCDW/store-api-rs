...
//     Table, Count, Id, CreateTime, UpdateTime, Qrcode, Name, Cover, Price, Unit
// }
...
pub fn list(page_number: usize, page_size: usize, name: Option<String>) -> Vec<Goods> {
    let sql = format!("SELECT count(*) FROM goods {} order by create_time desc LIMIT :page_size OFFSET :start", if name.is_none() { "" } else { "WHERE name like '%'||:name||'%'" });
